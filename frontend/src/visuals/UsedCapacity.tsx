...
export const UsedCapacityComponent: React.FC<UsedCapacityProps> = ({
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await fetchUsedCapacityMetrics(
        resourceGroup,
        storageAccount
      );
  ...
  const stats =
    data.length > 0
      ? {
          ...
          avg:
            data.reduce((sum, d) => sum + d.average, 0) / Math.max(data.length, 1),
        }
      : { min: 0, max: 0, avg: 0 };
